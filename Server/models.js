...
const addAQuestion = (params, callback) => {
  ...
  db.query('INSERT INTO questions(product_id, question_body, asker_name, email) VALUES ($1, $2, $3, $4)', queryArgs, (err) => {
    if (err) {
      callback(err);
    } else {
      callback(null);
...
const questionHelpful = (questionID, callback) => {
  ...
  db.query('UPDATE questions SET question_helpfulness = question_helpfulness + 1 WHERE question_id = $1', queryArgs, (err) => {
    if (err) {
      callback(err);
    } else {
      callback(null);
...
const questionReport = (questionID, callback) => {
  const queryArgs = [questionID];
  db.query('UPDATE questions SET reported = NOT reported WHERE question_id = $1', queryArgs, (err) => {
    if (err) {
      callback(err);
    } else {
      callback(null);
...
const getAnswers = (questionID, count = 5, callback) => {
  ...
  db.query(queryString, queryArgs, (err, answers) => {
    if (err) {
      callback(err);
    } else {
...
const answerHelpful = (answerID, callback) => {
  ...
  db.query('UPDATE answers SET helpfulness = helpfulness + 1 WHERE answer_id = $1', queryArgs, (err) => {
    if (err) {
      callback(err);
    } else {
      callback(null);
...
const answerReport = (answerID, callback) => {
  const queryArgs = [answerID];
  db.query('UPDATE answers SET reported = NOT reported WHERE answer_id = $1', queryArgs, (err) => {
    if (err) {
      callback(err);
    } else {
      callback(null);
...
module.exports = { getQuestions, addAQuestion, questionHelpful, questionReport, getAnswers, addAnswer, answerHelpful, answerReport}